...
from PyQt5.QtWidgets import QApplication, QMainWindow, QTabWidget, QWidget, QVBoxLayout, QPushButton, QLabel, QFileDialog, QDialog, QTableWidget, QTableWidgetItem
...
#             # layout.addWidget(self.plot_button)
    ...
    def show_metrics_popup(self, metrics):
        ...
        explanations = QLabel(
            "\nDegree Centrality: It reflects the number of direct connections a player has.\n"
            "Betweenness Centrality: It reflects the extent to which a player lies on the paths between other players.\n"
            "Closeness Centrality: It reflects how close a player is to all other players in the network.\n"
            "Eigenvector Centrality: It reflects a player's influence in the network based on connections.\n\n"
        )
        ...
        for row, (measure, (player, value)) in enumerate(metrics.items()):
            ...
            table.setItem(row, 0, QTableWidgetItem(measure_name))
            table.setItem(row, 1, QTableWidgetItem(player))
            table.setItem(row, 2, QTableWidgetItem(f"{value:.4f}"))
